...
class Agent:
    ...
    def action(self, **referee: dict) -> Action:
        ...
        # Play Phase: merge only on first move if no immediate eat available
        if not self._first_play_done:
            self._first_play_done = True
            can_eat = any(isinstance(a, EatAction) for a in self._legal_play_actions(self._color))
            if not can_eat:

...
DEPTH_LIM = 3   # odd -> this agents turn
...
class Agent:
    ...
    def action(self, **referee: dict) -> Action:
        # Placement is random for this agent
        if self._board.phase == GamePhase.PLACEMENT:
            return random.choice(self._legal_placements())

        # Play phase Evaluation - Minimax with Alpha-beta pruning
        alpha = -Math.inf
        bestMove = None
        for action in self._legal_play_actions(self._color):
            self._board.apply_action(action)
            eval = self._minimax(0,alpha,Math.inf)          # start with open window from the root
    ...
    # Minimax with alpha-beta pruning, searches up to DEPTH_LIM
